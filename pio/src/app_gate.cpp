/*
 * Gate runner — Arduino port for nRF52840 (RAK4630)
 * Replaces ESP-IDF logging, error macros, and FreeRTOS with Arduino equivalents.
 * Gate logic is preserved 1:1 from the ESP-IDF version.
 */
...
/* --- CONFIG_ to APP_ mapping helpers (Kconfig -> build flags) --- */

static bool gate3_expect_sgp40(void)
{
    return (APP_GATE3_EXPECTED_DEVICES == 1 || APP_GATE3_EXPECTED_DEVICES == 3);
}

static bool gate2p1_expect_sgp40(void)
{
    return (APP_GATE2P1_EXPECTED_DEVICES == 1 || APP_GATE2P1_EXPECTED_DEVICES == 3);
}

static bool gate2p1_expect_bmp280(void)
{
    return (APP_GATE2P1_EXPECTED_DEVICES == 2 || APP_GATE2P1_EXPECTED_DEVICES == 3);
}

static bool gate3_expect_bmp280(void)
{
    return (APP_GATE3_EXPECTED_DEVICES == 2 || APP_GATE3_EXPECTED_DEVICES == 3);
}

static bool gate4_expect_sgp40(void)
{
    return (APP_GATE4_EXPECTED_DEVICES == 1 || APP_GATE4_EXPECTED_DEVICES == 3);
}

static bool gate4_expect_bmp280(void)
{
    return (APP_GATE4_EXPECTED_DEVICES == 2 || APP_GATE4_EXPECTED_DEVICES == 3);
}

static bool gate9_expect_sgp40(void)
{
    return (APP_GATE9_EXPECTED_DEVICES == 1 || APP_GATE9_EXPECTED_DEVICES == 3);
}

static bool gate9_expect_bmp280(void)
{
    return (APP_GATE9_EXPECTED_DEVICES == 2 || APP_GATE9_EXPECTED_DEVICES == 3);
}
...
{
    switch (gate) {
    case APP_GATE_0_ENV:                return "env";
    case APP_GATE_1_HEARTBEAT:          return "heartbeat";
    case APP_GATE_2_DISPLAY_SMOKE:      return "display_smoke";
    case APP_GATE_2P1_I2C_SMOKE:        return "i2c_smoke";
    case APP_GATE_3_I2C_PRESENCE:       return "i2c_presence";
    case APP_GATE_4_SENSOR_PIPELINE:    return "sensor_pipeline";
    case APP_GATE_5_PAYLOAD_V1:         return "payload_v1";
    case APP_GATE_6_LORAWAN_JOIN_UPLINK: return "lorawan_join_uplink";
    case APP_GATE_7_RELIABILITY_BUFFER: return "reliability_buffer";
    case APP_GATE_8_FUOTA_SCAFFOLD:     return "fuota_scaffold";
    case APP_GATE_9_LIVE_PUBLISH:       return "live_publish";
...
{
    switch (gate) {
    case APP_GATE_0_ENV:                return "0";
    case APP_GATE_1_HEARTBEAT:          return "1";
    case APP_GATE_2_DISPLAY_SMOKE:      return "2";
    case APP_GATE_2P1_I2C_SMOKE:        return "2.1";
    case APP_GATE_3_I2C_PRESENCE:       return "3";
    case APP_GATE_4_SENSOR_PIPELINE:    return "4";
    case APP_GATE_5_PAYLOAD_V1:         return "5";
    case APP_GATE_6_LORAWAN_JOIN_UPLINK: return "6";
    case APP_GATE_7_RELIABILITY_BUFFER: return "7";
    case APP_GATE_8_FUOTA_SCAFFOLD:     return "8";
    case APP_GATE_9_LIVE_PUBLISH:       return "9";
    default:                            return "unknown";
    }
}
...
{
    gate_mark_pass(ctx, "toolchain_sanity=1 fw_build_date=%s fw_build_time=%s platform=nrf52840_arduino",
                   __DATE__, __TIME__);
}
...
{
    heartbeat_update();
    uint32_t toggles = heartbeat_get_toggle_count();
...
static void gate_tick_2p1(app_gate_ctx_t *ctx, uint64_t now_ms)
{
    ...
    esp_err_t err_sgp40 = i2c_bus_probe((uint8_t)APP_SGP40_ADDR, &sgp40);
    esp_err_t err_bmp280 = i2c_bus_probe((uint8_t)APP_BMP280_ADDR, &bmp280);

    if (err_sgp40 != ESP_OK || err_bmp280 != ESP_OK) {
        gate_log_fail(ctx, now_ms,
                      "i2c_smoke_probe_error expected=%d sgp40_err=%s bmp280_err=%s",
                      APP_GATE2P1_EXPECTED_DEVICES,
                      esp_err_to_name(err_sgp40), esp_err_to_name(err_bmp280));
        return;
    }

    if (expect_sgp40 && !sgp40) ctx->missing_sgp40_count++;
    if (expect_bmp280 && !bmp280) ctx->missing_bmp280_count++;

    gate_log_progress(ctx, now_ms,
                      "i2c_smoke expected=%d sgp40=%d bmp280=%d miss_sgp40=%lu miss_bmp280=%lu",
                      APP_GATE2P1_EXPECTED_DEVICES, sgp40, bmp280,
                      (unsigned long)ctx->missing_sgp40_count, (unsigned long)ctx->missing_bmp280_count);

    const bool pass_sgp40 = (!expect_sgp40 || sgp40);
    const bool pass_bmp280 = (!expect_bmp280 || bmp280);
    if (pass_sgp40 && pass_bmp280) {
        gate_mark_pass(ctx, "i2c_smoke_ok alias=2.1 expected=%d sgp40_addr=0x%02X bmp280_addr=0x%02X",
                       APP_GATE2P1_EXPECTED_DEVICES, APP_SGP40_ADDR, APP_BMP280_ADDR);
    }
}
...
static void gate_tick_3(app_gate_ctx_t *ctx, uint64_t now_ms)
{
    ...
    if (err_sgp40 != ESP_OK || err_bmp280 != ESP_OK) {
        gate_log_fail(ctx, now_ms, "probe_error sgp40_err=%s bmp280_err=%s",
                      esp_err_to_name(err_sgp40), esp_err_to_name(err_bmp280));
        return;
    }
    ...
    gate_log_progress(ctx, now_ms,
                      ...
                      APP_GATE3_EXPECTED_DEVICES, sgp40, bmp280,
                      (unsigned long)ctx->missing_sgp40_count, (unsigned long)ctx->missing_bmp280_count);

    const bool pass_sgp40 = (!expect_sgp40 || sgp40);
    const bool pass_bmp280 = (!expect_bmp280 || bmp280);
    if (pass_sgp40 && pass_bmp280) {
        gate_mark_pass(ctx, "presence_ok expected=%d sgp40_addr=0x%02X bmp280_addr=0x%02X",
                       APP_GATE3_EXPECTED_DEVICES, APP_SGP40_ADDR, APP_BMP280_ADDR);
    }
}
...
static void gate_tick_4(app_gate_ctx_t *ctx, uint64_t now_ms)
{
    ...
    if (expect_sgp40) {
        sensor_identity_t id_sgp40 = {};
        esp_err_t err = sensor_service_read_sgp40_identity(&id_sgp40);
        if (err != ESP_OK || !id_sgp40.sgp40_serial_ok) {
            gate_log_fail(ctx, now_ms, "identity_fail expected=%d sgp40_err=%s sgp40_serial_ok=%d",
                          APP_GATE4_EXPECTED_DEVICES, esp_err_to_name(err), id_sgp40.sgp40_serial_ok);
            return;
        }
    }

    if (expect_bmp280) {
        sensor_identity_t id_bmp280 = {};
        esp_err_t err = sensor_service_read_bmp280_identity(&id_bmp280);
        if (err != ESP_OK || !id_bmp280.bmp280_chip_id_ok) {
            gate_log_fail(ctx, now_ms, "identity_fail expected=%d bmp280_err=%s bmp280_chip_id_ok=%d bmp280_chip_id=0x%02X",
                          APP_GATE4_EXPECTED_DEVICES, esp_err_to_name(err),
                          id_bmp280.bmp280_chip_id_ok, id_bmp280.bmp280_chip_id);
            return;
        }
    }
    ...
    if (expect_sgp40 && expect_bmp280) {
        ...
        if (err != ESP_OK || !sample.valid) {
            ctx->sensor_fail++;
            gate_log_fail(ctx, now_ms, "sensor_read_fail expected=3 err=%s sensor_fail=%lu",
                          esp_err_to_name(err), (unsigned long)ctx->sensor_fail);
            return;
        }

        bool in_range = (sample.voc_index >= 1.0f && sample.voc_index <= 500.0f &&
                         sample.pressure_pa >= 80000.0f && sample.pressure_pa <= 120000.0f &&
                         sample.temperature_c >= -40.0f && sample.temperature_c <= 85.0f);
        if (!in_range) {
            ctx->sensor_fail++;
            gate_log_fail(ctx, now_ms, "range_fail expected=3 voc=%.2f pressure=%.1f temp=%.2f",
                          (double)sample.voc_index, (double)sample.pressure_pa, (double)sample.temperature_c);
            return;
        }
        ...
        gate_log_progress(ctx, now_ms, "sensor_ok_count=%lu expected=3 voc=%.2f pressure=%.1f temp=%.2f",
                          (unsigned long)ctx->sensor_ok, (double)sample.voc_index,
                          (double)sample.pressure_pa, (double)sample.temperature_c);
    } else if (expect_sgp40) {
        float voc_index = 0.0f;
        esp_err_t err = sensor_service_read_voc_only(&voc_index);
        if (err != ESP_OK) {
            ctx->sensor_fail++;
            gate_log_fail(ctx, now_ms, "voc_only_fail expected=1 err=%s sensor_fail=%lu",
                          esp_err_to_name(err), (unsigned long)ctx->sensor_fail);
            return;
        }
        if (voc_index < 1.0f || voc_index > 500.0f) {
            ctx->sensor_fail++;
            gate_log_fail(ctx, now_ms, "voc_range_fail expected=1 voc=%.2f", (double)voc_index);
            return;
        }
        ctx->sensor_ok++;
        gate_log_progress(ctx, now_ms, "sensor_ok_count=%lu expected=1 voc=%.2f",
                          (unsigned long)ctx->sensor_ok, (double)voc_index);
    } else {
        float pressure_pa = 0.0f;
        float temperature_c = 0.0f;
        esp_err_t err = sensor_service_read_bmp280(&pressure_pa, &temperature_c);
        if (err != ESP_OK) {
            ctx->sensor_fail++;
            gate_log_fail(ctx, now_ms, "bmp280_read_fail expected=2 err=%s sensor_fail=%lu",
                          esp_err_to_name(err), (unsigned long)ctx->sensor_fail);
            return;
        }
        if (pressure_pa < 80000.0f || pressure_pa > 120000.0f ||
            ...
            ctx->sensor_fail++;
            gate_log_fail(ctx, now_ms, "bmp280_range_fail expected=2 pressure=%.1f temp=%.2f",
                          (double)pressure_pa, (double)temperature_c);
            return;
        }
        ctx->sensor_ok++;
        gate_log_progress(ctx, now_ms, "sensor_ok_count=%lu expected=2 pressure=%.1fPa temp=%.2fC",
                          (unsigned long)ctx->sensor_ok, (double)pressure_pa, (double)temperature_c);
    }

    if (ctx->identity_ok && ctx->sensor_ok >= 3) {
        gate_mark_pass(ctx, "sensor_pipeline_ok expected=%d identity=1 sensor_ok=%lu sensor_fail=%lu",
                       APP_GATE4_EXPECTED_DEVICES,
                       (unsigned long)ctx->sensor_ok, (unsigned long)ctx->sensor_fail);
    }
}
...
static void gate_tick_5(app_gate_ctx_t *ctx, uint64_t now_ms)
{
    ...
    static const uint8_t expected[APP_PAYLOAD_V1_LEN] = {
        0x01, 0x27, 0x10, 0x00, 0x01, 0x8A, 0x24, 0x0B, 0x54, 0x0F, 0xA0, 0x01};

    uint8_t payload[APP_PAYLOAD_V1_LEN] = {0};
    size_t len = app_payload_encode_v1(&vector, payload, sizeof(payload));
...
static sensor_sample_t nominal_sample(uint64_t now_ms)
{
    sensor_sample_t s = {};
    s.voc_index = 101.1f;
    s.pressure_pa = 100950.0f;
    s.temperature_c = 28.9f;
    s.battery_v = sensor_service_read_battery_v();
    s.valid = true;
...
static void gate_tick_6(app_gate_ctx_t *ctx, uint64_t now_ms)
{
    ...
    gate_log_progress(ctx, now_ms,
                      ...
                      (unsigned long)m.join_attempts, lorawan_service_is_joined(),
                      (unsigned long)m.uplink_ok, m.backend_active, (unsigned long)m.retries);

    if (lorawan_service_is_joined() && m.uplink_ok >= 1) {
        gate_mark_pass(ctx, "joined=1 uplink_done=1 join_attempts=%lu backend_active=%d",
                       (unsigned long)m.join_attempts, m.backend_active);
    }
}
...
static void gate_tick_7(app_gate_ctx_t *ctx, uint64_t now_ms)
{
    ...
    if (!ctx->buffer_valid) {
        sensor_sample_t s = nominal_sample(now_ms);
    ...
    gate_log_progress(ctx, now_ms,
                      ...
                      (unsigned long)ctx->buffered_count, (unsigned long)ctx->flushed_count,
                      lorawan_service_is_joined(), (unsigned long)m.join_attempts, (unsigned long)m.retries);

    if (lorawan_service_is_joined() && ctx->flushed_count >= 1) {
        gate_mark_pass(ctx, "reliability_ok joined=1 buffered=%lu flushed=%lu uplink_ok=%lu",
                       (unsigned long)ctx->buffered_count, (unsigned long)ctx->flushed_count,
...
static void gate_tick_9(app_gate_ctx_t *ctx, uint64_t now_ms)
{
    ...
    if ((now_ms - ctx->last_sample_ms) >= (uint64_t)APP_GATE_SAMPLE_PERIOD_MS) {
        sensor_sample_t sample = {};
        ...
        if (err == ESP_OK && sample.valid) {
            ...
            if (display_service_should_refresh(
                    ...
                    &ctx->latest_sample, now_ms, ctx->last_refresh_ms)) {
            ...
            ctx->sensor_fail++;
            gate_log_fail(ctx, now_ms, "sample_read_fail expected=%d err=%s sensor_fail=%lu",
                          APP_GATE9_EXPECTED_DEVICES, esp_err_to_name(err),
                          (unsigned long)ctx->sensor_fail);
        }
        ctx->last_sample_ms = now_ms;
    }

    if (ctx->has_latest_sample && (now_ms - ctx->last_uplink_ms) >= (uint64_t)APP_GATE_UPLINK_PERIOD_MS) {
        ctx->latest_payload_len = app_payload_encode_v1(&ctx->latest_sample, ctx->latest_payload, sizeof(ctx->latest_payload));
    ...
    gate_log_progress(ctx, now_ms,
                      ...
                      lorawan_service_is_joined(), (unsigned long)m.uplink_ok, APP_GATE9_EXPECTED_DEVICES);

    if (ctx->sensor_ok >= 2 && ctx->display_updates >= 1 && m.uplink_ok >= 1) {
        gate_mark_pass(ctx, "live_publish_ok expected=%d sensor_ok=%lu display_updates=%lu uplink_ok=%lu",
...
{
    return (gate == APP_GATE_4_SENSOR_PIPELINE || gate == APP_GATE_9_LIVE_PUBLISH);
}
...
esp_err_t app_gate_init(app_gate_ctx_t *ctx)
{
    ...
    int selected = APP_GATE;
#if defined(APP_GATE_ID_SCHEME_LEGACY) && APP_GATE_ID_SCHEME_LEGACY
    selected = app_gate_translate_legacy_id(APP_GATE);
    ESP_LOGI(TAG, "gate_id_scheme=legacy input=%d resolved=%d", APP_GATE, selected);
#else
    ...
    ESP_LOGI(TAG, "boot board=RAK4630 gate=%s gate_raw=%d name=%s platform=nrf52840_arduino build=%s %s",
             app_gate_log_id(ctx->selected), (int)ctx->selected, app_gate_name(ctx->selected),
             __DATE__, __TIME__);
    ...
    if (gate_requires_sensor_init(ctx->selected)) {
        ...
        if (ctx->selected == APP_GATE_4_SENSOR_PIPELINE) {
            if (gate4_expect_sgp40() && !sensor_service_is_sgp40_present()) {
                ESP_LOGE(TAG, "gate4_missing_required_sgp40 expected_devices=%d", APP_GATE4_EXPECTED_DEVICES);
                return ESP_ERR_NOT_FOUND;
            }
            if (gate4_expect_bmp280() && !sensor_service_is_bmp280_present()) {
                ESP_LOGE(TAG, "gate4_missing_required_bmp280 expected_devices=%d", APP_GATE4_EXPECTED_DEVICES);
                return ESP_ERR_NOT_FOUND;
            }
        }
        ...
        if (ctx->selected == APP_GATE_9_LIVE_PUBLISH) {
            if (gate9_expect_sgp40() && !sensor_service_is_sgp40_present()) {
                ESP_LOGE(TAG, "gate9_missing_required_sgp40 expected_devices=%d", APP_GATE9_EXPECTED_DEVICES);
                return ESP_ERR_NOT_FOUND;
            }
            if (gate9_expect_bmp280() && !sensor_service_is_bmp280_present()) {
                ESP_LOGE(TAG, "gate9_missing_required_bmp280 expected_devices=%d", APP_GATE9_EXPECTED_DEVICES);
                return ESP_ERR_NOT_FOUND;
            }
        }
    }
...
void app_gate_tick(app_gate_ctx_t *ctx, uint64_t now_ms)
{
    ...
    switch (ctx->selected) {
    case APP_GATE_0_ENV:                gate_tick_0(ctx); break;
    case APP_GATE_1_HEARTBEAT:          gate_tick_1(ctx, now_ms); break;
    case APP_GATE_2_DISPLAY_SMOKE:      gate_tick_2(ctx); break;
    case APP_GATE_2P1_I2C_SMOKE:        gate_tick_2p1(ctx, now_ms); break;
    case APP_GATE_3_I2C_PRESENCE:       gate_tick_3(ctx, now_ms); break;
    case APP_GATE_4_SENSOR_PIPELINE:    gate_tick_4(ctx, now_ms); break;
    case APP_GATE_5_PAYLOAD_V1:         gate_tick_5(ctx, now_ms); break;
    case APP_GATE_6_LORAWAN_JOIN_UPLINK: gate_tick_6(ctx, now_ms); break;
    case APP_GATE_7_RELIABILITY_BUFFER: gate_tick_7(ctx, now_ms); break;
    case APP_GATE_8_FUOTA_SCAFFOLD:     gate_tick_8(ctx); break;
    case APP_GATE_9_LIVE_PUBLISH:       gate_tick_9(ctx, now_ms); break;
    default:                            gate_log_fail(ctx, now_ms, "invalid_gate_runtime"); break;
    }
}
