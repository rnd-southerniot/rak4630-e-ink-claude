...
| 9 `live_publish` | `APP_GATE=9`, devices=`2` (+creds) | PASS | Full cycle: live `bmp280_data` → `join_success` → `render_data voc=100.00 pressure=100051 temp=31.01 batt=3.95` → `uplink_sent_confirmed` → `uplink_ack_ok`; `result=PASS gate=9 sensor_ok=6 display_updates=1 uplink_ok=1`. Bench note: `APP_BATTERY_ADC_ENABLED=0` (no LiPo → default 3.95V); VOC seeded 100 (no SGP40). |
