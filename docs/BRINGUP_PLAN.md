...
- Gate 2.1: `i2c_smoke` (internal `CONFIG_APP_GATE=21`)
- Gate 3: `i2c_presence`
...
- Steps: set `CONFIG_APP_GATE=0`, build, flash, monitor.
...
- Steps: verify display pin mapping, set `CONFIG_APP_GATE=2`, run.
- Expected logs:
...
- Steps:
  ...
  2. Set `CONFIG_APP_GATE2P1_EXPECTED_DEVICES=<1|2|3>`.
  3. Set `CONFIG_APP_GATE=21` (Gate 2.1 alias), run.
- Expected logs:
  - `I2C: scan_found ...`
...
- Steps:
  ...
  2. Set `CONFIG_APP_GATE4_EXPECTED_DEVICES=<1|2|3>`.
  3. Set `CONFIG_APP_GATE=4`, run.
...
- Steps: set `CONFIG_APP_GATE=5`, run, verify hex + host tests.
...
- Steps:
  ...
  2. Set `CONFIG_APP_GATE9_EXPECTED_DEVICES=<1|2|3>`.
  3. Ask for `DEVEUI` + `APPKEY` in `firmware/.env`.
  4. Set `CONFIG_APP_GATE=9`, run.
- Expected logs:
