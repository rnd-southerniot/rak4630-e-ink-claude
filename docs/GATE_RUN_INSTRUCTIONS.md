...
1. Open `/Users/arif/rak4630-e-ink/docs/11-pin-mapping-rak3312-rak19007.md`.
...
```bash
perl -pi -e 's/^CONFIG_APP_GATE_ID_SCHEME_NEW=.*/CONFIG_APP_GATE_ID_SCHEME_NEW=y/; s/^# CONFIG_APP_GATE_ID_SCHEME_LEGACY is not set/# CONFIG_APP_GATE_ID_SCHEME_LEGACY is not set/; s/^CONFIG_APP_GATE=.*/CONFIG_APP_GATE=<0-9|21>/' /Users/arif/rak4630-e-ink/firmware/sdkconfig
```
...
```bash
perl -pi -e 's/^# CONFIG_APP_GATE_ID_SCHEME_LEGACY is not set/CONFIG_APP_GATE_ID_SCHEME_LEGACY=y/; s/^CONFIG_APP_GATE_ID_SCHEME_NEW=.*/# CONFIG_APP_GATE_ID_SCHEME_NEW is not set/; s/^CONFIG_APP_GATE_LEGACY=.*/CONFIG_APP_GATE_LEGACY=<0-8>/' /Users/arif/rak4630-e-ink/firmware/sdkconfig
```

## 4) Mandatory Gate Prompts

- Gate 1 (`heartbeat`): no sensor prompt. Heartbeat only.
  - Pin precheck: heartbeat primary `GPIO46` from `/Users/arif/rak4630-e-ink/docs/11-pin-mapping-rak3312-rak19007.md`.
...
```bash
perl -pi -e 's/^CONFIG_APP_GATE=.*/CONFIG_APP_GATE=1/' /Users/arif/rak4630-e-ink/firmware/sdkconfig
```
...
```bash
perl -pi -e 's/^CONFIG_APP_GATE=.*/CONFIG_APP_GATE=21/; s/^CONFIG_APP_GATE2P1_EXPECTED_DEVICES=.*/CONFIG_APP_GATE2P1_EXPECTED_DEVICES=<1|2|3>/' /Users/arif/rak4630-e-ink/firmware/sdkconfig
```
...
```bash
perl -pi -e 's/^CONFIG_APP_GATE3_EXPECTED_DEVICES=.*/CONFIG_APP_GATE3_EXPECTED_DEVICES=<1|2|3>/' /Users/arif/rak4630-e-ink/firmware/sdkconfig
```
...
```bash
perl -pi -e 's/^CONFIG_APP_GATE4_EXPECTED_DEVICES=.*/CONFIG_APP_GATE4_EXPECTED_DEVICES=<1|2|3>/' /Users/arif/rak4630-e-ink/firmware/sdkconfig
```
...
- Gate 6, 7, 9: ask for OTAA credentials in `/Users/arif/rak4630-e-ink/firmware/.env`:
  - `DEVEUI`
  - `APPKEY`
...
```bash
perl -pi -e 's/^CONFIG_APP_GATE9_EXPECTED_DEVICES=.*/CONFIG_APP_GATE9_EXPECTED_DEVICES=<1|2|3>/' /Users/arif/rak4630-e-ink/firmware/sdkconfig
```
