...
esp_err_t lorawan_service_init(void)
{
    ...
    s_metrics.backend_active = CONFIG_APP_LORAWAN_BACKEND_ACTIVE;
...
{
    if (s_state == LORAWAN_STATE_JOINED) {
        return;
    }

    if (now_ms < s_next_retry_ms) {
        return;
    }
    ...
    if (s_state == LORAWAN_STATE_IDLE) {
        ...
        if (!s_metrics.backend_active) {
            ...
            s_metrics.retries++;
            s_next_retry_ms = now_ms + (uint64_t)CONFIG_APP_LORAWAN_JOIN_BACKOFF_MS;
            ESP_LOGW(
                TAG,
                "join_attempt=%lu backend_active=0 retry_in_ms=%d",
                (unsigned long)s_metrics.join_attempts,
                CONFIG_APP_LORAWAN_JOIN_BACKOFF_MS);
            return;
        }
    ...
    if (s_state == LORAWAN_STATE_JOINING) {
        if (!s_metrics.backend_active) {
            ...
            s_metrics.retries++;
            s_next_retry_ms = now_ms + (uint64_t)CONFIG_APP_LORAWAN_JOIN_BACKOFF_MS;
            ESP_LOGW(TAG, "join_aborted backend_inactive retry_in_ms=%d", CONFIG_APP_LORAWAN_JOIN_BACKOFF_MS);
            return;
        }

        if (now_ms - s_join_started_ms >= (uint64_t)CONFIG_APP_LORAWAN_JOIN_SUCCESS_DELAY_MS) {
